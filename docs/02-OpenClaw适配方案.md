...
2. **Sub-agent**：读取授权的 state keys、`SKILL.md`、`prompts/`、`references/`，通过平台模型能力或本地 LangChain ChatModel tool-calling 生成 `state.intermediate.*`。
3. **Skill script**：只读取 intermediate，做 deterministic validation / formatting / rendering / persistence。
4. **本地模式**：模型配置沿用 `agent/llm_gateway.py`，Agent 决策使用 `agent/react/model_factory.py` 创建 LangChain ChatModel，并写入 `llm_trace.jsonl`。
...
# 3. 或使用本地 LangChain ReAct 验证 Main Agent → SubAgent graph → Skill
python -m agent run --request "我只需要一份关于 EMI 技术用于 CFRP 损伤检测的论文详细大纲。"
```
