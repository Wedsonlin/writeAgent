...
## 二、Skill 1 → Skill 2：`writing_task`
...
## 三、Skill 2 → Skill 3：`literature_report`
...
## 六、Skill 5 → Skill 6：`formatted_draft`
...
- **运行时**：每个 Skill 在写回 `state.json` 之前都用 `_shared.schemas` 里的
  pydantic 模型做 `model_validate`；失败时降级为"写入原始 payload + stderr 警告"，
  避免单次校验失败阻断流水线。
- **CI**：可选用 [check-jsonschema](https://check-jsonschema.readthedocs.io/) 在
  `tests/` 阶段做静态校验。
