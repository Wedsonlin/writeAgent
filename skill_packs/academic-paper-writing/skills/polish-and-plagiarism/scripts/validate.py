...
def validate_protected_claims(claims: Any, markdown: str) -> None:
    ...
    for index, claim in enumerate(claims):
        ...
        if text not in markdown:
            missing_fields.append(f"protected_claims[{index}]")

    if missing_fields:
        raise ContractError(
            "polished_markdown must preserve every protected_claims entry verbatim",
            missing_fields,
        )
...
def validate_blocking_contract(data: dict[str, Any], markdown: str) -> dict[str, Any]:
    ...
    validate_protected_claims(data.get("protected_claims"), markdown)
    return {
...
def compute_quality_checks(issues: list[Issue], polish_log: list[dict[str, Any]] | None = None) -> dict[str, bool]:
